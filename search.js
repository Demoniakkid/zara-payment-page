...
async function search(event) {
  ...
  if (
    searchProduct == "men shirts" ||
    searchProduct == "men shirt" ||
    ...
    searchProduct == "women accessories" ||
    searchProduct == "accessories"
  ) {
    searchProduct = "womenAccessories";
  } else if (
    searchProduct == "women jackets" ||
    searchProduct == "women jacket" ||
  ...
  let res = await fetch(
    `https://zara-server.herokuapp.com/products?section=${searchProduct}`
  );
  let products = await res.json();
  if (event.key == "Enter") {
